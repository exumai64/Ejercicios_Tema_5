...
def sustituye_palabras(texto: str, diccionario: dict[str, str]) -> str:
    ...
    ==> Se puede hacer con res.append(diccionario[p]), res.append[p], return " ".join(res).
    """
    ...
    for p in palabras:
        if p in diccionario:
            # si "p" está en el diccionario, la sustituimos
            res += diccionario[p] + " "
        else:
             res += p + " "
    return res.strip()
...
def calcula_distancia_media_frecuencias(freq1: dict[str, float], freq2: dict[str, float]) -> float:
    ...
    for bigrama in todas_claves:
        suma += abs(freq1.get(bigrama, 0) - freq2.get(bigrama, 0), 0)
